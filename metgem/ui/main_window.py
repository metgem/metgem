...
from metgem.models import metadata
from metgem import config, ui, utils
from metgem.workers import core as workers_core
from metgem.workers import gui as workers_gui
from metgem.workers import net as workers_net
from metgem.workers import databases as workers_dbs
from metgem.workers import options as workers_opts
from metgem.ui import widgets
from metgem.logger import logger, debug
from metgem.utils.network import Network, generate_id
from metgem.utils.emf_export import HAS_EMF_EXPORT

if HAS_EMF_EXPORT:
    from metgem.utils.emf_export import EMFPaintDevice
from metgem.utils.gui import enumerateMenu, SignalGrouper, SignalBlocker
from metgem.config import get_python_rendering_flag
from metgem.utils import hasinstance
from metgem.ui.main_window_ui import Ui_MainWindow
from metgem.ui.widgets.search_ui import Ui_Form as Ui_SearchWidget
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.actionFindStandards.triggered.connect(lambda: self.on_query_databases('show',
                                                                                   {node.index() for node in
                                                                                    self.current_view.scene().selectedNodes()}))
        # noinspection PyPep8
                    ...
                    if new_options != options:
                        ...
                        if widget.name == widgets.ForceDirectedFrame.name:
                            ...
                            self._workers.append(lambda _: self.prepare_generate_network_worker(widget,
                                                                                                new_options,
                                                                                                keep_vertices=True))
                        self._workers.append(lambda _: widget.create_draw_worker(new_options))
                        self._workers.append(lambda _: self.update_status_widgets())
                        self._workers.start()
                        self.update_search_menu()
    ...
    @debug
    def prepare_generate_network_worker(self, widget, options=None, keep_vertices=False):
        options = self._network.options.get(widget.id, None) if options is None else options
        if options is None:
            return
    ...
    def prepare_export_db_results_worker(self, filename: str, values,
                                         ...
                                         selected_rows) -> workers_gui.ExportDbResultsWorker:
