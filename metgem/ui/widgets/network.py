...
from metgem.ui.widgets.network_ui import Ui_NetworkFrame
...
class BaseFrame(QFrame, Ui_NetworkFrame):
    ...
    def create_draw_worker(self, options=None, compute_layouts=True, colors=[], radii=[], layout=None, isolated_nodes=None):
        scene = self.scene()
        if self.use_edges:
        ...
        if compute_layouts or self._layout is None:
            ...
            return [self.create_worker(options), process_finished]
        else:
