...
def compute_scores_from_spectra(spectra, use_multiprocessing=False, tqdm=tqdm):
    '''Generate a network from a MGF file.
    
    Args:
        filename (str): path of MGF file to process.
    '''

    #%% Create scores matrix
    print('Allocating matrix...', end=' ')
    t0 = time.time()
    ...
    print('Done in {}'.format(pretty_time_delta(time.time() - t0)))


    #%% Compute cosine scores
    with tqdm(total=num_scores_to_compute) as pbar:
        pbar.set_description('Computing cosine scores...')
        pbar.refresh()
        
        if use_multiprocessing:
            combinations = itertools.combinations(range(num_spectra), 2)
            max_workers = mp.cpu_count()
            pool = mp.Pool(max_workers, initializer=init, initargs=(spectra, num_spectra, scores_matrix))
            ...
            combinations = itertools.combinations(spectra, 2)
    ...
    #%% Fill matrices with predictable values
...
if __name__ == '__main__':
    # Config
    FILENAME = 'codiaeum mzmine3.mgf'
    # FILENAME = 'SFE 3 GapF.mgf'
    # FILENAME = 'test Q-ToF.mgf'
    OUTPUT_PREFIX = 'COSINE_SUM_'
    ...
    if USE_MULTIPROCESSING:
        SCORES_OUTPUT = OUTPUT_PREFIX + f + '.csv'
    else:
        SCORES_OUTPUT = OUTPUT_PREFIX + f + '_single.csv'
        
    NETWORKEDGES_OUTPUT = 'networkedges_selfloop_' + f
    CLUSTERINFOS_OUTPUT = 'clusterinfosummarygroup_attributes' + f
    ...
    t0 = time.time()
    spectra = list(read_mgf(FILENAME, USE_MULTIPROCESSING))
    print('Done in {}'.format(pretty_time_delta(time.time() - t0)))

    
    #%% Generate network
    scores_matrix = compute_scores_from_spectra(spectra, USE_MULTIPROCESSING)
    
    
    #%% Save scores to a CSV file
    print('Saving scores to {}...'.format(SCORES_OUTPUT), end=' ')
    t0 = time.time()
    np.savetxt(SCORES_OUTPUT, scores_matrix, delimiter='\t', fmt='%.6f')
    print('Done in {}'.format(pretty_time_delta(time.time() - t0)))

    
    #%% Writing network edges to a file
    print('Saving network edges to {}...'.format(NETWORKEDGES_OUTPUT), end=' ')
    t0 = time.time()
    
    interactions = generate_network(scores_matrix, spectra)
    print(interactions)
    
    with open(NETWORKEDGES_OUTPUT, 'w') as f:
        f.write('Source\tTarget\tDelta Spectrum.MZ\tCosine\n')
        
        for source, target, delta, score in interactions:
            f.write('{:d}\t{:d}\t{:f}\t{:.6f}\n'.format(source+1, target+1, delta, score))
            
    print('Done in {}'.format(pretty_time_delta(time.time() - t0)))
    
    
    #%% Writing Cluster infos to a file
    print('Saving cluster infos to {}...'.format(CLUSTERINFOS_OUTPUT), end=' ')
    t0 = time.time()
    with open(CLUSTERINFOS_OUTPUT, 'w') as f:
        f.write('Source\tparent mass\n')
        for i, spectrum in enumerate(spectra):
            f.write('{:d}\t{:.3f}\n'.format(i+1, spectrum.mz_parent))
    print('Done in {}'.format(pretty_time_delta(time.time() - t0)))
    
    
    #%% Print total time
    print('All done in {}'.format(pretty_time_delta(time.time() - t00)))
